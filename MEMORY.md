...
- Writing on the photo must be readable. Official ₿ must be a clean mark. Never stamp a slogan on a flat chest rectangle, and never leave a ghost of the old line (HODL / 21 MILLION) under the new one. Recolour must not smash letters. If you cannot read the title at card size, do not list the SKU. Hoodie/pullover restamp: `node scripts/render-sweat-mockups.mjs`.
