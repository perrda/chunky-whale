...
export default async function SuccessPage({ searchParams }: Props) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-16 md:px-6">
      {order ? <ClearCart /> : null}
      <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-gold">
        {order?.demo ? "Demo order" : "Order received"}
      </p>
      <h1 className="mt-2 font-display text-4xl font-extrabold">The stamp is in motion.</h1>
      {order ? (
        <>
          <p className="mt-4 font-mono text-sm text-ember">{order.id}</p>
          <p className="mt-4 font-serif text-paper/80">
            {order.demo
              ? "Payment providers are not connected yet, so this is a practice order. Nothing was charged. When Stripe, OpenNode, and Coinbase keys are in .env, this screen will follow a real payment."
              : `Pay with ${order.method}. We will email ${order.email} when Printful starts the print.`}
          </p>
          <ul className="mt-8 space-y-2 font-serif">
            {order.items.map((i) => (
              <li key={`${i.slug}-${i.size}`}>
                {getProduct(i.slug)?.name} {i.size ? `(${i.size})` : ""} × {i.qty} — {formatGbp(i.priceGbp * i.qty)}
              </li>
            ))}
          </ul>
          ...
          {order.method === "bitcoin" && order.demo ? (
            <div className="mt-8 border border-paper/15 bg-surface p-6">
              ...
              <p className="mt-2 font-serif text-sm text-paper/75">
                Live mode shows an OpenNode invoice (on-chain address + Lightning QR). At Bitcoin MENA the same flow sits on a tablet as POS.
              </p>
              <div className="mt-4 grid place-items-center border border-dashed border-ember/50 p-8">
                ...
                <p className="mt-3 font-mono text-[10px] uppercase tracking-[0.2em] text-gold">Invoice QR placeholder</p>
              </div>
            </div>
          ) : null}
          {order.method === "usdt" && order.demo ? (
            <div className="mt-8 border border-paper/15 bg-surface p-6">
              ...
              <p className="mt-2 font-serif text-sm text-paper/75">
                Live mode opens a NOWPayments invoice priced in GBP, paid in USDT (TRC-20 by default). USDC remains on Coinbase Commerce.
              </p>
            </div>
          ) : null}
