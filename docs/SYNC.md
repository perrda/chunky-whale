...
On the shop page, **More → Mummy & Daddy** must show the parent pieces, not “Nothing matches.” Drinkware is whiskey and shot glasses only (failed mugs stay retired). Checkout total must match items + the shipping line. Checkout still says **demo** until your keys are in. `/fulfillment` should send you to Shipping — that page must not talk about booths.

Open [HODL Pullover](http://127.0.0.1:3001/product/hodl-pullover). You must be able to read **I AM HODLING** and see a clean orange ₿ — not a black box or broken letters. Then tap Bone: the shirt goes cream and the writing stays readable. Under the photos, **This line** should show the same joke on other objects. Open [Bitcoin Daddy Hoodie](http://127.0.0.1:3001/product/bitcoin-daddy-hoodie) — **BITCOIN DADDY** must be readable with a clean ₿ on the chest. Open [Bitcoin Daddy Tee](http://127.0.0.1:3001/product/bitcoin-daddy-tee) — **BITCOIN DADDY** in clean type, orange ₿ leaning **right** (not the old 3D B). Then [₿ Mark Hoodie](http://127.0.0.1:3001/product/b-mark-hoodie) — same lean as the header coin. Drinkware must not show a ₿ floating off a white mug (those listings are retired).
