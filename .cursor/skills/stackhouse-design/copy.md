# Copy bank

Voice: a Bitcoiner explaining it to a clever friend. Funny, serious, and giftable. Always Bitcoin.

## Meme line (lead the catalog)

- I AM HODLING — Dec 2013 BitcoinTalk typo
- STACK SATS — small units, long game
- FEW UNDERSTAND
- BITCOIN FIXES THIS
- 1 BTC = 1 BTC
- 21 MILLION
- DON'T TRUST. VERIFY.
- THERE IS NO SECOND BEST
- HARD MONEY — house mark
- SATOSHI WAS HERE
- PEER-TO-PEER ELECTRONIC CASH — white paper title
- GRADUALLY, THEN SUDDENLY
- NOT YOUR KEYS NOT YOUR COINS
- INFINITE FIAT / FINITE BITCOIN
- RUN YOUR NODE
- NUMBER GO UP — rising line + ₿, not FOMO21 “models destroyed”
- WE ARE SO BACK / IT'S SO OVER — cycle mood, not a price call
- STRATEGIC RESERVE — nation-state joke, not advice
- NO LASER EYES
- PROOF OF WORK NOT PROOF OF TWEET
- FIAT IS THE EXPERIMENT
- QUANTUM CAN WAIT
- ORANGE PILL
- NOT A FORECAST
- FOUR YEAR TIDE
- THE PRINTER IS THE JOKE
- IN CASE IT CATCHES ON
- NOTHING STOPS THIS TRAIN
- WHEN IN DOUBT, ZOOM OUT
- THIS IS GOOD FOR BITCOIN
- SATS ARE THE STANDARD
- VIRES IN NUMERIS
- BITCOIN IS HOPE
- I LOST IT IN A BOATING ACCIDENT
- INFLATION IS THEFT
- IT'S GOING UP FOREVER, LAURA — joke, not a forecast
- BUY THE DIP — cycle mood, not advice
- THE HOUSE ALWAYS STACKS
- BLOCK ZERO
- VERIFY THEN HODL
- SOUND MONEY LOUD SHIRT
- CHUNKY WHALE EST. 2009

House list (add / remove here and in `lib/sayings.json`): [docs/SAYINGS.md](../../../docs/SAYINGS.md)  
Energy vs clones: [docs/DESIGN-DIRECTION.md](../../../docs/DESIGN-DIRECTION.md)

## Charts as jokes

- Log scale (the punchline is the scale)
- The dip is the feature (V recovery)
- I stayed for the chart
- Difficulty adjusts
- Ten minutes (block interval)

## Family

- Orange. Daily. — amber bottle, ₿ as the pill
- Node in training — infant onesie, tiny ₿
- Future UTXO — youth, small ₿
- Same Bitcoin B as adult line so the family matches in photos

## Event capsules

- MENA / Amsterdam / Prague / Nashville — ₿ + city, not a tourist tee

## Never print (FOMO21 / clones)

First Rule Of Bitcoin, B Sovereign, All Your Models Are Destroyed, Vitamin B, Tick Tock Next Block, Fix The Money Fix The World, Fist Bitcoin, Get Rich Or Die Mining, HFSP, Make Money Great Again, Only The Paranoid Survive, Going To Zero Since 2009, Melt Your Face Off, got bitcoin?, Nirvana Satoshi, MAGA / Barbie / Metallica / Top Gun / Banksy / Pulp Fiction / Mt. Gox / laser-eye clones. Steal their **energy** (varied grid, quiet next to loud). Never their art. See `docs/DESIGN-DIRECTION.md`.
