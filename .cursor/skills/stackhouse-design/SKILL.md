...
- Storefront UX: phone menu sections stay closed until Show. Header search suggests live products. Collection/shop lists sort (featured / price / name). Product cards show the cut. Product page: one colour picker, zoom, quantity, breadcrumbs. Basket/checkout use colour **labels** (Navy, not `navy`) and a shipping estimate. Checkout charges **items + shipping** via `basketTotals()` — never items only. Shop chips call `productsIn()` so Mummy & Daddy / Drinkware / Sweatshirts stay in sync with collection pages.
...
6. Run `npm run qa`. It must print `0 error(s)` on catalog, storefront filters, and payment helpers. Do not ship if it fails.
...
- **HARD RULE — writing and marks must be readable.** The buyer decides from the photo. Every live shot must show the title line in clean, complete letters and a coherent official ₿ **on the object** — not smashed type, missing strokes, a second ghost slogan, a sticker rectangle, or a ₿ hanging off a mug/hat into the white backdrop. If you cannot read it at card size, or the mark is off the product, **retire the SKU**. Never ship a garbled “I AM HODLING”. Hoodie/pullover restamps: `node scripts/render-sweat-mockups.mjs` (clone fabric, erase the old chest line, then stamp). Do not paint a flat colour box over the chest. Do not composite a new ₿ onto an existing white-mug photo — that leaves a floating mark or a sticker box. Wait for a new unique studio shot.
