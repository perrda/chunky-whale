...
- `npm run qa:catalog` (also runs before `next build`) fails the build if two live designs share a photo, a mug/tote/pint uses the wrong object, a live photo is not a white studio mockup, a live photo still has an **upright** Bitcoin B, a swatch recolour does not match the garment colour (`color-match`), the studio backdrop is grainy (`grain`), or a live photo is not renderer output (`print-source`).
- **HARD RULE — writing and marks must be readable.** The buyer decides from the photo. Every live shot must show the title line in clean, complete letters and a coherent official ₿ **on the object** — not smashed type, missing strokes, a second ghost slogan, a sticker rectangle, or a ₿ hanging off a mug/hat into the white backdrop. If you cannot read it at card size, or the mark is off the product, **retire the SKU**. Never ship a garbled “I AM HODLING”. Live apparel/glass photos must come from `scripts/render-tee-mockups.mjs`, `scripts/render-sweat-mockups.mjs`, or `scripts/render-glass-mockups.mjs` (official ₿ via `garmentMarkPng` + Inter). The only photo exception is `21m-hat.png` (real embroidered dad hat). Do not paint a flat colour box over the chest. Do not composite a new ₿ onto an existing white-mug or hat photo — that leaves a floating mark or a sticker box. Failed swim, totes, jewelry, posters, leftover hats/mugs stay retired until a clean unique studio shot exists.
